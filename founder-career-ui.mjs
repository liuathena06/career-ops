...
function inferenceLine(item){const value=typeof item?.value==='object'?(item.value.searchTitle||JSON.stringify(item.value)):item?.value;return [value,item?.confidence&&'confidence: '+item.confidence,item?.status&&'status: '+item.status,item?.rationale].filter(Boolean).join(' | ')}
function renderFounderReview(data){reviewContent.replaceChildren();const stated=data.profile?.stated||{},constraints=stated.hardConstraints||{},compensation=constraints.compensation||{};const profileRows=[['Career direction',values(stated.careerDirection).join('；')],['Motivations',values(stated.motivations).join('；')],['Priorities',values(stated.priorities).join('；')],['Locations',values(constraints.locations).join('；')],['Deal breakers',values(constraints.dealBreakers).join('；')],['Minimum compensation',compensation.minimum?.confirmed?String(compensation.minimum.value):'—'],['Capability evidence',values(stated.capabilityEvidence).join('；')],['Differentiators',values(stated.selfDescribedDifferentiators).join('；')]].map((item)=>item[0]+': '+(item[1]||'—'));reviewList('Confirmed Career Profile · user stated facts',profileRows);const realQwen=data.intelligenceRun?.analyzer==='bailian-qwen-career-intelligence'&&!data.intelligenceRun?.usedFallback;const run=data.intelligenceRun||{};reviewList('AI provider diagnostics',['provider requested: '+(run.providerRequested||'mock'),'Qwen credential available: '+String(Boolean(run.qwenCredentialAvailable)),'Qwen call success: '+String(Boolean(run.qwenCallSuccess)),'fallback used: '+String(Boolean(run.usedFallback)),'sanitized failure reason: '+(run.sanitizedFailureReason||'none')]);reviewList('AI provenance',[realQwen?'真实 Qwen 调用：已完成':'本地 deterministic/mock fallback：已使用',realQwen?'以下 AI 内容均为 pending inference，不改变 confirmed Profile。':'以下内容不是 Qwen 输出，不改变 confirmed Profile。']);const label=realQwen?'Qwen inference · pending confirmation':'Local analysis · pending confirmation';reviewList(label+' — career thesis',[data.intelligence?.careerThesis?.text||'—']);reviewList(label+' — evidenced capabilities',(data.intelligence?.coreCapabilities||[]).map(inferenceLine));reviewList(label+' — transferable capabilities',(data.intelligence?.transferableCapabilities||[]).map(inferenceLine));reviewList('Career directions · stated vs inference',(data.intelligence?.nextStageDirections||[]).map((item)=>item.kind+': '+item.searchTitle+' | '+(item.source==='confirmed_profile'?'confirmed stated direction':'pending inference')+(item.reason?' | '+item.reason:'')));reviewList('Generated Liepin search strategy',(data.searchStrategy?.searches||[]).map((item)=>item.kind+': '+item.jobName+(item.location?' · '+item.location:'')+(item.explorationOnly?' · exploration':'')+(item.reason?' | '+item.reason:'')));reviewList('Uncertainties / pending confirmation',[...(data.intelligence?.warnings||[]),...(data.intelligence?.needsConfirmation||[]).map(inferenceLine)]);}
const extras={
...
function confirmedValues(entries) { return (entries ?? []).filter((entry) => entry?.confirmed && typeof entry.value === 'string' && entry.value.trim()).map((entry) => entry.value.trim()); }
