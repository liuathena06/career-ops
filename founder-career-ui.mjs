...
<section class="step"><h2>1. Resume Upload</h2><input id="resume" type="file" accept=".pdf,.doc,.docx,.txt,.md"><label for="resumeText">简历文本（TXT/MD 会自动读取；PDF/DOC 可粘贴文本）</label><textarea id="resumeText" placeholder="仅保留在本次本地会话；不保存、不上传到第三方。"></textarea><p id="resumeStatus" class="hint">简历只保留在当前浏览器会话。</p></section>
...
<script>
const questions=${QUESTIONS}; const interview=document.querySelector('#interview'),profileStatus=document.querySelector('#profileStatus'),searchStep=document.querySelector('#searchStep'),searchStatus=document.querySelector('#searchStatus'),results=document.querySelector('#results'),resumeTextInput=document.querySelector('#resumeText'); let profile=null,intelligence=null,searchStrategy=null; let resumeDiagnostic={upload:"not_read",fileType:"unknown",parse:"not_started",language:"unknown",characters:0};
const extras={
...
document.querySelector('#resume').addEventListener('change',async(e)=>{const f=e.target.files?.[0];if(!f){resumeDiagnostic={upload:"not_read",fileType:"unknown",parse:"not_started",language:"unknown",characters:0};document.querySelector("#resumeStatus").textContent="未选择简历。";return}const name=f.name.toLowerCase();const readable=f.type.startsWith("text/")||name.endsWith(".txt")||name.endsWith(".md");resumeDiagnostic={upload:"success",fileType:f.type||name.split(".").pop()||"unknown",parse:"fail",language:"unknown",characters:0};if(readable){try{const content=await f.text();const letters=(content.match(/[A-Za-z]/g)||[]).length;resumeDiagnostic={...resumeDiagnostic,parse:content.length>0?"success":"fail",language:letters>0?"English":"non-English/unknown",characters:content.length};resumeTextInput.value=content}catch{}}document.querySelector("#resumeStatus").textContent="Resume upload: "+resumeDiagnostic.upload+" | parse: "+resumeDiagnostic.parse+" | language: "+resumeDiagnostic.language+" | characters: "+resumeDiagnostic.characters;});
...
createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === '/api/profile') { const founded = createFounderConfirmedProfile({ answers: input.answers }); const fallbackAnalyzer = createMockCareerIntelligenceAnalyzer(); const analyzer = isBailianQwenConfigured() ? createBailianQwenCareerIntelligenceAnalyzer() : fallbackAnalyzer; const analyzed = await analyzeCareerIntelligence({ analyzer, fallbackAnalyzer: analyzer === fallbackAnalyzer ? null : fallbackAnalyzer, resumeText: input.resumeText, interview: founded.interview, profile: founded.profile }); const intelligence = analyzed.intelligence; return json(res, 200, { ...founded, intelligence, intelligenceRun: { analyzer: analyzed.analyzer.id, usedFallback: analyzed.usedFallback }, searchStrategy: createOpportunitySearchStrategy({ profile: founded.profile, intelligence }) }); }
