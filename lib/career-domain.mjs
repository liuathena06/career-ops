...
function emptyStated() {
  return Object.fromEntries(PROFILE_STATED_FIELDS.map((field) => [field, []]));
}
...
function validateStated(stated) {
  if (!isPlainObject(stated)) fail('stated must be an object');
  for (const field of PROFILE_STATED_FIELDS) {
    if (!Array.isArray(stated[field])) fail(`stated.${field} must be an array`);
    validateUniqueIds(stated[field], `stated.${field}`);
    stated[field].forEach((entry, index) => validateStatedEntry(entry, `stated.${field}[${index}]`));
  }
...
function assertEvidenceReferencesAnsweredTurns(stated, inferences, interview) {
  ...
  const allEntries = [
    ...PROFILE_STATED_FIELDS.flatMap((field) => stated[field]),
    ...inferences,
  ];
...
export function createRecruiterRubricCandidateInput(profile) {
  ...
  const hardConstraints = confirmed
    ? stated.hardConstraints.filter((entry) => entry.confirmed)
    : [];
